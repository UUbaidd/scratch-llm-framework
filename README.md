...
Then we generate matrices for Q, K , V using Linear class. Here we use uniform distribution and for uniform distibution we first need to find boundry. So when we find min and max bound then every number has equal chance. We use uniform distribution for weight initialisation because we want randomness. Then we perform **Wx+b**
